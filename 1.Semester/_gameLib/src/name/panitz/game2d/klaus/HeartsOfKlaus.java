...
public record HeartsOfKlaus(FallingImage player, int width, int height, List<List<? extends GameObj>> goss,
	...
	static ImageObject newHeart(Vertex corner) {
		return new ImageObject(corner, new Vertex(0, 0), "banana.gif");
	}

	static ImageObject newWall(Vertex corner) {
		return new ImageObject(corner, new Vertex(0, 0), "BLACK.png");
	}

	static final int GRID_WIDTH = 35;

	static Level[] levelList = new Level[2];

	static Level crlevel;
	...
	public void doChecks() {
		if(won()){
			readLevel(crlevel.getNextLevel());
		}
		collectHearts();
	...
	public void init() {
		...
		crlevel = new Level(level1);
		Level secLevel = new Level(level2);
		secLevel.setNextLevel(crlevel);
		crlevel.setNextLevel(secLevel);
	...
	private void readLevel(Level crlevel) {
		int l = 0;
		var lines = crlevel.getLevelData().split("\\n");
